...
def make_question(
    ...
    """
    options_data can be:
    1) list of strings [optA, optB, optC, optD]
    2) list of tuples [(text, trap, mistake), ...]
    """
    options = []
    opt_ids = ["A", "B", "C", "D"]
    for i, opt_id in enumerate(opt_ids):
        ...
        if isinstance(item, tuple):
            ...
            opt_text = item["text"]
            trap = item.get("trap", None if is_corr else "Conceptual Misapplication")
            mistake = item.get("mistake", "Correct NCERT principle" if is_corr else "Incorrect application of NCERT fact")
        else:
            opt_text = str(item)
            trap = None if is_corr else "Distractor Choice Trap"
            mistake = (
                "Correct answer. Consistent with NCERT Biology principles."
                if is_corr
...
def make_sequence_question(
    ...
    for i, opt_id in enumerate(opt_ids):
        is_corr = (opt_id == correct_opt)
        opt_str = options_list[i]
        trap = None if is_corr else (traps[i] if traps else "Chronological Inversion Trap")
        mistake = (
            "Correct answer. Follows the precise biological chronological sequence."
            if is_corr
            else "Inverts or scrambles the intermediate biological stages."
        )
        opts.append({
            "id": opt_id,
            "text": opt_str,
...
def make_statement_question(
    ...
    for i, opt_id in enumerate(opt_ids):
        ...
        opts.append({
            ...
            "isCorrect": is_corr,
            "studentSelectionTrap": None if is_corr else trap,
            "mistakeAnalysis": "Correct answer. Correctly evaluates truth value of both statements according to NCERT." if is_corr else mistake
        })
