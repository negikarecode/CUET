...
for m in range(1, 21):
    ...
    for q in questions:
        ...
        qid = q["id"]
        # ID check
        assert qid == f'fa-mock{m}-q{qnum}', f'Bad ID: {qid}'
        assert q['questionId'] == f'fa-mock{m}-q{qnum}', f'Bad questionId: {qid}'
        
        # QA Metadata
        cid = q['conceptId']
        assert cid.startswith('fa.'), f'Bad conceptId: {cid}'
...
print('ALL FINE ARTS AUDITS PASSED PERFECTLY!')
