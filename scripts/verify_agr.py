...
for m in range(1, 21):
    path = f'mock/agriculture/{m}.json'
    ...
    for q in questions:
        ...
        qid = q["id"]
        # ID check
        assert qid == f'agr-mock{m}-q{qnum}', f'Bad ID: {qid}'
        assert q['questionId'] == f'agr-mock{m}-q{qnum}', f'Bad questionId: {qid}'
        
        # QA Metadata
        cid = q['conceptId']
        assert cid.startswith('agr.'), f'Bad conceptId: {cid}'
...
print('ALL AGRICULTURE AUDITS PASSED PERFECTLY!')
