...
for m in range(1, 21):
    path = f'mock/environmental_studies/{m}.json'
    ...
    for q in questions:
        ...
        qid = q["id"]
        # ID check
        assert qid == f'evs-mock{m}-q{qnum}', f'Bad ID: {qid}'
        assert q['questionId'] == f'evs-mock{m}-q{qnum}', f'Bad questionId: {qid}'
        
        # QA Metadata
        cid = q['conceptId']
        assert cid.startswith('evs.'), f'Bad conceptId: {cid}'
...
print('ALL EVS AUDITS PASSED PERFECTLY!')
