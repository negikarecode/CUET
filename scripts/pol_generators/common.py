...
def get_pyq_normalized_set():
    pyq_set = set()
    pyq_file = "scripts/parsed_pol_science_pyqs.json"
...
DEFAULT_POL_TRAPS = [
    ("Chronological Inversion Trap", "Confuses the exact sequence of historical treaties, elections, or political accords."),
    ("Institutional Misattribution Trap", "Confuses powers or jurisdictions between different international or domestic organs (e.g. UN General Assembly vs Security Council, Planning Commission vs NITI Aayog)."),
    ("Ideological / Party Confusion Trap", "Attributes a political stance, manifesto promise, or slogan to the wrong party or political faction."),
    ("Constitutional Article Misalignment Trap", "Confuses constitutional amendments, articles, or emergency provisions."),
    ("Geopolitical Entity Conflation Trap", "Conflates regional agreements, member states, or founding treaties (e.g. ASEAN declarations vs SAARC summits).")
]
...
def rotate_options(correct_text, wrong_texts, target_id, solution_template, mistake_analysis_correct, wrong_traps_and_mistakes=None):
    ...
    if wrong_traps_and_mistakes is None:
        wrong_traps_and_mistakes = DEFAULT_POL_TRAPS
...
def make_question(
    ...
    if options_data and isinstance(options_data[0], dict) and "id" in options_data[0]:
        ...
        for i, opt_id in enumerate(opt_ids):
            ...
            if isinstance(item, tuple):
                ...
                mistake = item.get("mistake", "Correct NCERT Political Science concept" if is_corr else "Incorrect application")
            else:
                opt_text = str(item)
                trap = None if is_corr else "Distractor Choice Trap"
                mistake = (
                    "Correct answer. Conforms to standard NCERT Political Science syllabus."
                    if is_corr
                    else f"Incorrect option. Confuses political science concepts with distractor '{opt_text}'."
                )
    ...
    return {
        "id": f"pol-q-{question_number}-{abs(hash(text)) % 1000000}",
